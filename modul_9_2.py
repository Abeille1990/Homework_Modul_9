...
a = first_strings + second_strings # верное решение, но можно упросить без определения отделно переменной - см. third_result2
third_result1 = {x: len(x) for x in a if len(x) % 2 == 0}
print(third_result1)

third_result2 = {x: len(x) for x in first_strings + second_strings if len(x) % 2 == 0}
print(third_result2)
